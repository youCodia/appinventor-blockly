...
goog.provide('Blockly.ComponentDatabase');
 ...
 */
var ComponentTypeDescriptor = function() {};
 ...
 */
var ComponentInstanceDescriptor = function() {};
...
Blockly.ComponentDatabase.prototype.forEachInstance = function(callback) {
  Object.values(this.instances_).forEach(callback);
};
...
Blockly.ComponentDatabase.prototype.forEventInType = function(typeName, callback) {
  if (typeName in this.types_) {
    Object.keys(this.types_[typeName].eventDictionary).map(function(key) {
      return callback(this.types_[typeName].eventDictionary[key], key);
    });
  }
};
...
Blockly.ComponentDatabase.prototype.forMethodInType = function(typeName, callback) {
  if (typeName in this.types_) {
    Object.keys(this.types_[typeName].methodDictionary).map(function(key) {
      return callback(this.types_[typeName].methodDictionary[key], key);
    });
  }
};
